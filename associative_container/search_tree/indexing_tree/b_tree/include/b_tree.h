...
//     auto path = this->find_path(lower_bound); //1-node, 2- index
//     if (path.top().second < 0)
//     {
//         throw std::logic_error("Key not found");
//     }
...
//     return (*path.top().first)->keys_and_values[path.top().second].value;
    ...
    auto it = infix_iterator(this->_root);
    while ((it != this->end_infix()) && (this->_keys_comparer(upper_bound, std::cref(std::get<2>(*it))) > (upper_bound_inclusive
                                                                                                                  ? -1
                                                                                                                  : 0)))
    {
        range.push_back(typename associative_container<tkey, tvalue>::key_value_pair(std::cref(std::get<2>(*it)), std::cref(std::get<3>(*it))));
        ++it;
    }
