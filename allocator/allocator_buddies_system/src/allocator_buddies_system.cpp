...
// TODO: wtf?!
    ...
    *space_bool_target = 0;
    ...
    void* buddie = get_buddie(target_block);
    if (buddie != nullptr)
    {
        ...
        while (buddie != nullptr && power_buddie == power_target && is_occupied_buddie == 0)
        {
            ...
            *next_block_current = next_block_buddie;
            ...
            buddie = get_buddie(target_block);
            if (buddie != nullptr)
            {
    ...
    allocator_with_fit_mode::fit_mode mode)
{
    ...
    log_with_guard_my("start set_fit_mode", logger::severity::trace);
...
{
    log_with_guard_my("start get_fit_mode", logger::severity::trace);
...
{
    log_with_guard_my("start get_allocator", logger::severity::trace);
...
void* allocator_buddies_system::get_buddie(void* address_block)
{
    size_t size_block = 1 << *reinterpret_cast<short*>(address_block);
    size_t addr = reinterpret_cast<size_t>(address_block);
    size_t buddie = addr ^ size_block;
    return reinterpret_cast<void*>(buddie);
}
