...
protected:
    ...
    {
    friend class binary_search_tree<int, std::string>;
    protected:
    ...
    tvalue &&value)
{
    ...
    this->_tree->call_node_constructor(*(path.top()) = reinterpret_cast<node *>(allocate_with_guard_my(this->_tree->get_node_size(), 1)), key, std::move(value));
